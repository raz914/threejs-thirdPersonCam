...
class ThreeJSScene {
  ...
  setupLighting() {
    ...
    // Directional light with expanded shadow area
    const dirLight = new THREE.DirectionalLight(0xffffff, 2);
    dirLight.position.set(-30, 100, -60);
    dirLight.castShadow = true;
    dirLight.shadow.camera.top = 200;  // Increased from 20 to 200
    dirLight.shadow.camera.bottom = -200;  // Increased from -20 to -200
    dirLight.shadow.camera.left = -200;  // Increased from -20 to -200
    dirLight.shadow.camera.right = 200;  // Increased from 20 to 200
    dirLight.shadow.camera.near = 0.1;
    dirLight.shadow.camera.far = 1000;  // Already set to a high value

    // Optional: Increase shadow map resolution for better quality shadows
    dirLight.shadow.mapSize.width = 8192;  // Increased from 2048 to 4096
    dirLight.shadow.mapSize.height = 8192; // Default is 512
